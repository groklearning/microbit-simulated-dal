...
/**
  ...
  *
  * Represents the device as a whole, and includes member variables to that reflect the components
 * of the system.
  */
class MicroBit {
 private:
 ...
 public:
  ...
  // Array of components which are iterated during idle thread execution, isIdleCallbackNeeded is
  // polled during a systemTick.
  ...
  /**
    ...
    *
    * @note Values of 6 and below tend to lose resolution - do you really need to sleep for this
  ...
  /**
    ...
    * @return A random, natural number between 0 and the max-1. Or MICROBIT_INVALID_PARAMETER if max
   * is <= 0.
    *
    * Example:
    * @code
  ...
  /**
    * System tasks to be executed by the idle thread when the Micro:Bit isn't busy or when data
   * needs to be read.
    */
  void systemTasks();

  /**
    * add a component to the array of system components which invocate the systemTick member
   * function during a systemTick
    *
    * @param component The component to add.
    * @return MICROBIT_OK on success. MICROBIT_NO_RESOURCES is returned if further components cannot
   * be supported.
    */
  ...
  /**
    ...
    * @return MICROBIT_OK on success. MICROBIT_INVALID_PARAMETER is returned if the given component
   * has not been previous added.
    */
  int removeSystemComponent(MicroBitComponent* component);

  /**
    * add a component to the array of of idle thread components.
    * isIdleCallbackNeeded is polled during a systemTick to determine if the idle thread should jump
  ...
  /**
    ...
    *
    * @return The time since the last reset, in milliseconds. This will result in overflow after 1.6
   * months.
  ...
  /**
    * Triggers a microbit panic where an infinite loop will occur swapping between the panicFace and
   * statusCode if provided.
    *
    * @param statusCode the status code of the associated error. Status codes must be in the range
   * 0-255.
    */
